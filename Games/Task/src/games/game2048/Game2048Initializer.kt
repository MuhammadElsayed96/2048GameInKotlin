...
object RandomGame2048Initializer : Game2048Initializer<Int> {
    ...
    override fun nextValue(board: GameBoard<Int?>): Pair<Cell, Int>? {
        return if (board.all { it != null }) null
        else {
            val randValue = generateRandomStartValue()
            val s = board.filter { it == null }.toMutableList()
            val randCell = Random.nextInt(s.size)
            Pair(s[randCell], randValue)
        }
    }
}